...
class errorClass
	{
		...
		function err_chkUploadValue($val,$valText)
		{
			if(strlen($val) <= 0 ||  $val == "0")
			{
				$this->errMsg .= "* Please Upload " . $valText . ".<br>";
			}else
			{
				$this->checkAbuseLanguage($val,$valText);
			}
		...
		function err_chkCharacter($val,$valText,$char)
		{
			if(strlen($val) <= 0)	
			{	
				$this->errMsg .= "* Please Enter " . $valText . ".<br>";
			...
			{		
				$this->checkAbuseLanguage($val,$valText);	  				 
			 	$val	=	stripslashes($val);
				if(!ereg("^[/ A-Za-z0-9,".$char."]+$", $val, $arr))
				{					
					$this->errMsg .= "* ".$valText." should not contain special characters expect $char.<br>";
			 	} 
		...
		function err_chkTitle($val,$valText)
		{
			if(strlen($val) <= 0)	
			{	
				$this->errMsg .= "* Please Enter " . $valText . ".<br>";
			...
			{		
				...
				if(!ereg("^[/ A-Za-z0-9,)( -]+$", $val, $arr))
				{					
					$this->errMsg .= "* ".$valText." should not contain special characters expect ().<br>";
			 	} 
		...
		function err_chkAlphanumericText_category($val,$valText)
		{
			if(strlen($val) <= 0)	
			{	
				$this->errMsg 	.= "* Please Enter " . $valText . ".<br>";
		...
		function err_chkTextLength($val,$valText,$length)
		{
			if(strlen($val))
			{
				...
				{
					$this->errMsg .= "* ". ucfirst($valText) ." should be at least " . $length . " character long.<br>";
				}				
			} 
			return($this->errMsg);
		}
	...
	// --------- Check validation wih unique key --------
		...
		function err_chkUniqueTxtNew($tblName,$chkfield1,$val,$chkfield2,$val2,$oper,$valText,$db)
		{
			...
			{	
			 ...
			 {	
				 ...
				 if($oper == 0)
				  ...
				  if($oper == 1)
				  {	
					 if($db->CheckDupExIndex($tblName,$chkfield1,$val,$chkfield2,$val2) == 1)	
					 {	
							$this->errMsg .= "* " . $valText . " already exists, please try with other name.<br>";							
					}
				  }
		...
		function err_chkPhoneValidations($val,$valText)
		{
			if(strlen($val)<=0)
			{
				$this->errMsg .= "* Please Enter ". $valText .". <br>";
		...
		function err_chkPageIndex($val,$part,$dir,$tblname,$fieldChkVal,$fieldChk,$pgFieldName,$db,$valText)
		{			
			//$filelen	=	strlen(basename($dir));
			//$dir	=	substr($dir,0,-($filelen+1));
			...
			else
			{
				...
				else
				{
					...
					if (($db->CheckDupExIndex($tblname,$pgFieldName,$val,$fieldChkVal,$fieldChk) != 0))
					{
		...
		function err_chkPageForumIndex($val,$part,$dir,$tblname,$fieldChkVal,$fieldChk,$pgFieldName,$db,$valText)
		{			
			//$filelen	=	strlen(basename($dir));
			//$dir	=	substr($dir,0,-($filelen+1));
			...
			else
			{
				...
				else
				{
					...
					if (($db->CheckDupExForumIndex($tblname,$pgFieldName,$val,$fieldChkVal,$fieldChk) != 0))
					{
...
/* function err_chkSelect_max checks validation for a select list 
where you can select maximum $max options at a time 
inputs: select-list name(it requires in array format)
		Text to show
		Maximum limit
Added by Bhagyashree*/

		 function err_chkSelect_max($val,$valText,$max)
		 {
			if(sizeof($val) > $max)
			{
				$this->errMsg	.= "* You can select maximum ".$max." ".$valText.".<br>";
			}			
			return $this->errMsg;		
		 }
		  function err_maxLimit_Order($val,$valText,$max)
		 {
			if(sizeof($val) < $max)
			{
				$this->errMsg	= "* Please enter correct ".$valText." Max order is  ".$max."<br>";
			}			
			return $this->errMsg;		
		 }
		 
//end function err_chkSelect_max
		 ...
		 {			
			 if(strlen($val) > 0 )
			 {
				...
				if(  strcasecmp($flsavechk,"png")!=0 && strcasecmp($flsavechk,"jpg")!=0 && strcasecmp($flsavechk,"jpeg")!=0 && strcasecmp($flsavechk,"gif")!=0 && strcasecmp($flsavechk,"swf")!=0 )
		...
		// Video validation
		 function err_chkVideoType($val,$valText,$valSize)
		 {			
			 if(strlen($val) > 0 )
			 {
				$flsavechk = strtolower(substr(strrchr($val, "."), 1));
				if(  strcasecmp($flsavechk,"mpeg")!=0 && strcasecmp($flsavechk,"mpg")!=0 && strcasecmp($flsavechk,"dat")!=0 && strcasecmp($flsavechk,"wmv")!=0)
				{
					$this->errMsg .= "* Please Select valid ".$valText." of mpeg/mpg/dat/wmv format.<br>";	
				}
				else
				{
					if($valSize > 5000000 ) //104857600 
						$this->errMsg .= "*  Video File size exceeds limit.<br>";	
				}
...
//			echo "<BR>k  ".$oper;
		...
		/* when url in not cumpulsary */
		function err_chkUrlOnly($val,$valText)
		{
			if(strlen($val) > 0)
			{
				$CHECK_URL 			= TRUE;
		...
		{
			//Rroved rape & bastard from list as per client requirement by sayali 12:50 PM 3/21/2011
			 $abuseKeyWords = "asshole:fuck:benchod:madarchod";
		...
		// --------- Check validation wih unique key --------
		function err_chkUniqueTextWithLevel($tblName,$chkfield1,$val,$chkfield2,$val2,$oper,$valText,$db,$level)
		{			
			if(strlen($val) <= 0)	
			{	
				$this->errMsg	    .= "* Please Enter " . $valText . ".<br>";
			...
			{	
				$count					= 0;
				$specialChArr			= array("`", "!", "#", "$", ":", "%", "^", "&", "*", "(", ")","_","+","=","~","\\","/","@","|",".",",",";");
				str_replace($specialChArr,"",$val,$count);
	 			if( $count > 0 )
				{
					$this->errMsg .= "* ".ucfirst($valText)." should not contain special characters .<br>";
				}else
				{
					if($oper == 0)		
					 {
						if($db->CheckDupWithLevel($tblName,$chkfield1,$val,$level) == 1)
							$this->errMsg .= "* The " . $valText . " already taken, Please try with other.<br>";							
					 }
					 else
						if($oper == 1)
						{				
							if($db->CheckDupExWithLevel($tblName,$chkfield1,$val,$chkfield2,$val2,$level) == 1)						 
								$this->errMsg .= "* The " . $valText . " already taken, Please try with other.<br>";							
						}
				}//else
			} 
		...
		function err_chkUniqueEmail($tblName,$chkfield1,$val,$chkfield2,$val2,$oper,$valText,$db)
		{			
			if(strlen($val) <= 0)	
			{	
				$this->errMsg	    .= "* Please Enter " . $valText . " .<br>";
				 ...
				 if($oper == 0)
				  {
					...
					if($db->CheckDup($tblName,$chkfield1,$val) == 1)
						$this->errMsg .= "* " . $valText . " already exists.<br>";							
				  }
				  else
				  if($oper == 1)
				  {				
		...
		function err_chkUrl($val,$valText)
		{			
			if(strlen($valText) <=0) 	
				$valText	=	"URL";
			...
			{				
				if( !preg_match('/^(http:\/\/)+(([\w]+:)?\/\/)?(([\d\w]|%[a-fA-f\d]{2,2})+(:([\d\w]|%[a-fA-f\d]{2,2})+)?@)?([\d\w][-\d\w]{0,253}[\d\w]\.)+[\w]{2,4}(:[\d]+)?(\/([-+_~.\d\w]|%[a-fA-f\d]{2,2})*)*(\?(&amp;?([-+_~.\d\w]|%[a-fA-f\d]{2,2})=?)*)?(#([-+_~.\d\w]|%[a-fA-f\d]{2,2})*)?$/',$val) )
				{
					$this->errMsg	.= "* Please Enter Valid ".$valText." Including http://<br>";
				}
			}
			...
			return($this->errMsg);
                ...
                function err_chkNumeric_fixLength($val,$valText,$length)
		{
			if(strlen($val)<=0)
			{
				$this->errMsg .= "* Please Enter ". $valText .". <br>";
